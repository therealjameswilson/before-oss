...
test("Batch 195 preserves Bernard through Bernstein rows and publishes evidence without forced matches", async ({
  page,
}) => {
  const profiles = [
    ["8a2732d5-406a-5a2e-9495-61666e8d5216", "Jules E Bernard", "Capt", "masked"],
    ["126863d3-c4ba-5454-b6e8-042bcb2227bc", "Melba M Bernard", "Caf-2", "Not printed"],
    ["2f65487f-af42-5c89-a64c-a4a1c599acfe", "Gerard J Bernier", "SP X 2/c", "masked"],
    ["3453f26f-bd11-51c9-a92c-658252ee5866", "Jean D Bernier", "pVT", "masked"],
    ["67b26ea3-7bd3-5458-bf94-0890b44be6be", "Veronica Bernier", "Not printed", "Not printed"],
    ["beb1fd12-49ae-5704-b9d0-126afa311a78", "Roger P Bernique", "1st Lt", "masked"],
    ["4a4271f3-0fe0-5c8f-9719-40d78b8ea752", "Harry L Berno", "Col", "masked"],
    ["d754ef5b-d0e7-50c0-af46-c32ec809c796", "Philip B Berns", "T-4", "masked"],
    ["c50080d8-c033-5367-bed3-5611db1aca60", "Aaron D Bernstein", "Not printed", "Not printed"],
    ["2e4ae6dc-8d90-5c3d-ba42-25180d32a2d5", "Blanche Bernstein", "P-6", "Not printed"],
  ];
  ...
  await page.goto("./people/3453f26f-bd11-51c9-a92c-658252ee5866/");
  await expect(page.getByText("confirmed", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("occupation only found", { exact: true }).first()).toBeVisible();
  await expect(page.locator("main")).toContainText("Waiter or waitress, except private family");
  await expect(page.locator('section[aria-labelledby="civilian-employer"]')).toContainText(
    "No reliable pre-OSS employer has yet been identified",
  );

  await page.goto("./people/beb1fd12-49ae-5704-b9d0-126afa311a78/");
  await expect(page.getByText("high confidence", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("documented prewar employer found", { exact: true }).first()).toBeVisible();
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "Lawyer in private practice",
  );
  await expect(page.locator("main")).toContainText("New Bedford");
  await expect(page.locator('section[aria-labelledby="civilian-employer"]')).toContainText(
    "No reliable pre-OSS employer has yet been identified",
  );

  await page.goto("./people/4a4271f3-0fe0-5c8f-9719-40d78b8ea752/");
  await expect(page.getByText("high confidence", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("documented prewar employer found", { exact: true }).first()).toBeVisible();
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "W. H. Davie Seal Company",
  );
  await expect(page.locator("main")).toContainText("Vice president");
  await expect(page.locator("main")).toContainText("1934");

  await page.goto("./people/d754ef5b-d0e7-50c0-af46-c32ec809c796/");
  await expect(page.getByText("conflicting", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("conflicting sources", { exact: true }).first()).toBeVisible();
  await expect(page.locator("main")).toContainText("Donald J. Kreusch");
  await expect(page.locator("main")).toContainText("identifier itself is withheld");

  for (const personId of [
    "8a2732d5-406a-5a2e-9495-61666e8d5216",
    "126863d3-c4ba-5454-b6e8-042bcb2227bc",
    "2f65487f-af42-5c89-a64c-a4a1c599acfe",
    "67b26ea3-7bd3-5458-bf94-0890b44be6be",
    "c50080d8-c033-5367-bed3-5611db1aca60",
    "2e4ae6dc-8d90-5c3d-ba42-25180d32a2d5",
  ]) {
    await page.goto(`./people/${personId}/`);
    await expect(page.getByText("unresolved", { exact: true }).first()).toBeVisible();
    await expect(page.getByText("requires archival review", { exact: true }).first()).toBeVisible();
...
test("Batch 212 publishes qualified page 37 findings without overstating employers", async ({
  page,
}) => {
  const profiles = [
    ["1e53f7c2-d215-5dbe-9534-0a8962e48a74", "Adolf Birnbaum", "T-5", true],
    ["d08091bc-0935-5e6b-bbad-ce0b3f4e1887", "Rachel S Birnbaum", "CAF-4", false],
    ["124244b4-fc84-5fe7-a15d-fe16dba9e07a", "Marian F Birrell", "Caf-5", false],
    ["46a1de83-e5b8-5a68-bc6d-25987ef93e32", "Edward M Birtcil Sr.", "Pvt", true],
    ["42f3847d-83ba-50d2-9c6f-8a2546d521ee", "Vincent J Bisaccia", "Pfc", true],
    ["49595fcc-64dc-5aff-b693-f7e71be4bb55", "Marcel Biscaino", "S/Lt", false],
    ["4b9481f3-2063-55fc-957c-14932c16891a", "Bruno T Bisceglia", "1st Lt", true],
    ["067efbcf-afde-5738-b807-01f39ae64956", "Harry J Bishman", "Cpl", false],
    ["00e2f454-0b01-5bbe-bd82-e148f492fd30", "Charles W Bishop", "Capt", true],
    ["a4e3e2d2-f1be-55c9-9165-ea921eec049b", "Florence S Bishop", "SP-5", false],
  ] as const;
  ...
  await page.goto("./people/1e53f7c2-d215-5dbe-9534-0a8962e48a74/");
  await expect(page.getByText("confirmed", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("occupation only found", { exact: true }).first()).toBeVisible();
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "Stock clerk",
  ...
  await page.goto("./people/42f3847d-83ba-50d2-9c6f-8a2546d521ee/");
  await expect(page.getByText("confirmed", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("occupation only found", { exact: true }).first()).toBeVisible();
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "Student; institution not identified",
  ...
  await page.goto("./people/49595fcc-64dc-5aff-b693-f7e71be4bb55/");
  await expect(page.getByText("high confidence", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("requires archival review", { exact: true }).first()).toBeVisible();
  await expect(page.locator("main")).toContainText("Marcel Biscaïno");
  await expect(page.locator('section[aria-labelledby="civilian-employer"]')).toContainText(
    "No reliable pre-OSS employer has yet been identified",
  );

  await page.goto("./people/4b9481f3-2063-55fc-957c-14932c16891a/");
  await expect(page.getByText("probable", { exact: true }).first()).toBeVisible();
  await expect(
    page.getByText("documented prewar employer found", { exact: true }).first(),
  ).toBeVisible();
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "B-Line Transport Company",
  );
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "Bisceglia family wine business",
  );
  await expect(page.locator('section[aria-labelledby="immediate-affiliation"]')).toContainText(
    "No reviewed claim currently meets the publication threshold",
  );
  ...
  for (const personId of [
    "d08091bc-0935-5e6b-bbad-ce0b3f4e1887",
    "124244b4-fc84-5fe7-a15d-fe16dba9e07a",
    "46a1de83-e5b8-5a68-bc6d-25987ef93e32",
    "067efbcf-afde-5738-b807-01f39ae64956",
    "00e2f454-0b01-5bbe-bd82-e148f492fd30",
    "a4e3e2d2-f1be-55c9-9165-ea921eec049b",
  ]) {
    await page.goto(`./people/${personId}/`);
    await expect(page.getByText("unresolved", { exact: true }).first()).toBeVisible();
    await expect(page.getByText("requires archival review", { exact: true }).first()).toBeVisible();
...
test("Batch 240 preserves duplicate rows and separates occupations, military pathways, and employers", async ({
  page,
}) => {
  const profiles = [
    ["a6c2db1a-f89d-5b02-bc1a-28d478c1b5cc", "Geopffrey H Bonnell", "43", "67"],
    ["67023822-cd73-5f2e-8943-e2e9d977e203", "Albert Bonner", "43", "67"],
    ["e9071f5c-7b89-5b19-8ba4-4f006c24fc2e", "Charles W Bonner", "43", "67"],
    ["4e50cf87-dd29-54b3-a86c-e813a2e26cea", "Douglas G Bonner", "43", "67"],
    ["fa197a07-db7a-5a98-981b-f956c53a8f28", "Francis W Bonner", "43", "68"],
    ["cbe19c19-6a10-511f-aa88-e16088a2eb26", "Henry S Bonner", "43", "68"],
    ["363f399b-87c0-5352-b634-85d06fd8e8d1", "Henry S Bonner", "43", "68"],
    ["3cc073cd-b4f3-5770-9015-66efddc6ef49", "Robert A Bonner", "43", "68"],
    ["c2110fba-b86f-5d7f-ad59-e4cd80eee95c", "Robert N Bonner", "43", "68"],
    ["df6fc9be-1074-568c-b4e4-d1839917086e", "Emile V Bonnet", "44", "68"],
  ] as const;

  for (const [personId, displayName, pageNumber, box] of profiles) {
    await page.goto(`./people/${personId}/`);
    await expect(page.getByRole("heading", { name: displayName, exact: true })).toBeVisible();
    await expect(page.locator(".index-record").first()).toContainText(`Page ${pageNumber}`);
    await expect(page.locator(".index-record").first()).toContainText(box);
  ...
  await page.goto("./people/a6c2db1a-f89d-5b02-bc1a-28d478c1b5cc/");
  await expect(page.getByText("high confidence", { exact: true }).first()).toBeVisible();
  await expect(page.locator("main")).toContainText("commissioned army officer");
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "Florida-West Indies Airway",
  );
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
    "Wall Street",
  );
  ...
  for (const [personId, occupation] of [
    ["67023822-cd73-5f2e-8943-e2e9d977e203", "farm hand on a general farm"],
    [
      "df6fc9be-1074-568c-b4e4-d1839917086e",
      "sports instructor, athlete, or sports official",
    ],
  ] as const) {
    await page.goto(`./people/${personId}/`);
    await expect(page.getByText("confirmed", { exact: true }).first()).toBeVisible();
  ...
  await page.goto("./people/4e50cf87-dd29-54b3-a86c-e813a2e26cea/");
  await expect(page.getByText("probable", { exact: true }).first()).toBeVisible();
  await expect(page.getByText("needs identity review", { exact: true }).first()).toBeVisible();
  await expect(page.locator('section[aria-labelledby="earlier-affiliations"]')).toContainText(
  ...
  for (const personId of [
    ...
    "fa197a07-db7a-5a98-981b-f956c53a8f28",
    "cbe19c19-6a10-511f-aa88-e16088a2eb26",
    "363f399b-87c0-5352-b634-85d06fd8e8d1",
    "3cc073cd-b4f3-5770-9015-66efddc6ef49",
    "c2110fba-b86f-5d7f-ad59-e4cd80eee95c",
  ]) {
    await page.goto(`./people/${personId}/`);
    await expect(page.getByText("unresolved", { exact: true }).first()).toBeVisible();
    await expect(
  ...
  for (const personId of [
    ...
    await page.goto(`./people/${personId}/`);
    await expect(page.locator("main")).toContainText("two visually verified, separately printed");
    await expect(page.locator("main")).toContainText("do not merge them on name alone");
  }

  await page.goto("./people/3cc073cd-b4f3-5770-9015-66efddc6ef49/");
  await expect(page.locator("main")).toContainText("commissioned army officer");
