...
audit passes all seven structural checks. Astro checks 209 source files with
zero errors, warnings or hints and builds 24,646 direct HTML pages. The
bounded browser release suite passes **75 / 75** checks across desktop,
phone and tablet, including 27 axe cases. All internal links resolve; 50,119
external URLs are inventoried, not comprehensively visited. Public identifier
auditing examines 12,926 normalized private identifiers and 120 formatted
...
- Batch 506 local QA: all **94 / 94** Python tests pass. SQLite `quick_check`
  ...
  passes all seven checks. Extraction validation covers all 522 pages and
  23,978 source rows; all 32 parser warnings are visually resolved, and 92
  selected pages have been reviewed. Twenty-one focused Batch 506 checks, 57
  bounded core checks and 27 axe cases pass across desktop, phone and tablet.
  Astro checks 130 source files without errors, warnings or hints and generates
  24,547 HTML pages / 24,619 artifacts. Every internal target resolves; 49,872
  external URLs are inventoried. The identifier audit examines 12,926
  normalized identifiers, 120 formatted variants and 1,098 artifact candidates
  with zero leaks. The production dependency audit finds zero vulnerabilities.
  Consecutive builds reproduce public-tree digest
  ...
  See `reports/release_batch_506.md`.

- Batch 505 local QA: all **94 / 94** Python tests pass. SQLite `quick_check`
  returns `ok`, foreign keys pass, and the deterministic 200-profile audit
  passes all seven checks. Extraction validation covers all 522 pages and
  23,978 source rows; all 32 parser warnings are visually resolved, and 92
  selected pages have been reviewed. Twenty-one focused Batch 505 checks, 42
  bounded core checks and 27 axe cases pass across desktop, phone and tablet.
  Astro checks 129 source files without errors, warnings or hints and generates
  24,546 HTML pages / 24,618 artifacts. Every internal target resolves; 49,869
  external URLs are inventoried. The identifier audit examines 12,926
  normalized identifiers, 120 formatted variants and 1,102 artifact candidates
  with zero leaks. The production dependency audit finds zero vulnerabilities.
  Consecutive builds reproduce public-tree digest
  ...
  See `reports/release_batch_505.md`.
...
- Batch 493 local QA: all **94 / 94** Python tests plus 43 subtests pass.
  ...
  covers all 522 pages and 23,978 source rows. Fifteen focused Batch 493
  checks pass across desktop, phone and tablet; the separate accessibility
  suite passes **27 / 27** across the same viewports. The initial focused run
  exposed three test-only wording expectations; the corrected suite passed
  without changing evidence or application behavior. Astro checks 117 source
  ...
  49,797 unique external URLs. Five of eight current-batch endpoints return
  HTTP 200; three return HTTP 403 to the generic checker, while their relevant
  metadata or page text had already been reviewed through permitted browser
  access. The public-identifier audit examines 12,926 normalized identifiers
  and 120 formatted variants, rejects 1,101 substring coincidences, and finds
  ...
  See `reports/release_batch_493.md`.

- Batch 492 local QA: all **94 / 94** Python tests plus 43 subtests pass.
  SQLite `quick_check` returns `ok`, foreign keys pass, and the deterministic
  200-profile structural audit passes all seven checks. Extraction validation
  covers all 522 pages and 23,978 source rows. Eighteen focused Batch 492
  checks pass across desktop, phone and tablet. The main axe route run passed
  26/27; its one tablet Sources-page timeout had no axe finding and passed 1/1
  on a clean isolated rerun, giving every one of the 27 cases a passing final
  result. Astro checks 116 source files with zero errors, warnings or hints and
  generates 24,516 HTML pages / 24,588 artifacts. The internal-link audit
  resolves every page and inventories 49,794 unique external URLs. Five of six
  current-batch endpoints return HTTP 200; the Columbia handle redirects to a
  HathiTrust endpoint that returned HTTP 403 to the checker, while the exact
  report page was separately downloaded from a public mirror and visually
  checked. The public-identifier audit examines 12,926 normalized identifiers
  ...
  See `reports/release_batch_492.md`.
...
- Batch 490 local QA: all **94 / 94** Python tests plus 43 subtests pass.
  SQLite `quick_check` returns `ok`, foreign keys pass, and the deterministic
  200-profile structural audit passes all seven checks. Extraction validation
  covers all 522 pages and 23,978 source rows. Fifteen focused Batch 490 checks
  pass across desktop, phone and tablet; the complete browser and accessibility
  matrix passes **2,853 / 2,853** in 26.3 minutes with retries disabled. Astro
  checks 114 source files with zero errors, warnings or hints and generates
  24,516 HTML pages / 24,588 artifacts. The internal-link audit resolves every
  page and inventories 49,792 unique external URLs. All five current-batch
  source endpoints return HTTP 200. The public-identifier audit examines
  12,926 normalized identifiers and 120 formatted variants, rejects 1,099
  substring coincidences, and finds zero aggregate, manifest-size or
  unexpected boundary matches. The production dependency audit finds zero
  vulnerabilities. Consecutive projections and Pages builds reproduce public-
  tree digest
  ...
  See `reports/release_batch_490.md`.
...
- Batch 414 complete browser and accessibility matrix: **1,782 / 1,782
  ...
  disabled. The clean run includes all 27 axe cases and all twelve new Batch
  414 cases; the focused regression separately passed **12 / 12** on the final
  build. All **90 / 90** Python tests pass. The database passed `quick_check`,
  had zero foreign-key violations, and passed the deterministic 200-profile
  audit. Extraction validation passed all 522 pages and 23,978 rows. The
  public-identifier audit examined 12,926 normalized private identifiers and
  120 formatted variants across 24,532 production artifacts, rejected 1,082
  substring coincidences, and found zero aggregate, manifest-size, or
  unexpected boundary matches. All internal links resolve across 24,460 HTML
  pages; 49,589 unique external URLs are inventoried, not all visited. The
  public manifest covers 67 assets totaling 84,019,625 bytes. Five of seven
  Batch 414 citation endpoints returned HTTP 200; Hoover and Library of
  Congress returned HTTP 403 after their exact source images had been visually
  inspected, and no access control was bypassed. The dependency audit found
  zero vulnerabilities. Consecutive exports reproduced public-data digest
...
- Batch 413 complete browser and accessibility matrix: **1,770 / 1,770
  ...
  disabled. The clean run includes all 27 axe cases and all nine new Batch 413
  cases; the focused regression separately passed **9 / 9** on the final
  build. All **90 / 90** Python tests pass. The database passed `quick_check`,
  had zero foreign-key violations, and passed the deterministic 200-profile
  audit. Extraction validation passed all 522 pages and 23,978 rows. The
  public-identifier audit examined 12,926 normalized private identifiers and
  120 formatted variants across 24,531 production artifacts, rejected 1,087
  substring coincidences, and found zero aggregate, manifest-size, or
  unexpected boundary matches. All internal links resolve across 24,459 HTML
  pages; 49,587 unique external URLs are inventoried, not all visited. The
  public manifest covers 67 assets totaling 83,929,419 bytes. All four Batch
  413 citation endpoints returned HTTP 200, and the dependency audit found
  zero vulnerabilities. Consecutive exports reproduced public-data digest
...
- Batch 389 complete browser and accessibility matrix: **1,446 / 1,446
  passed** in 11.2 minutes across desktop, phone, and tablet. The clean,
  uninterrupted run includes all 27 axe cases and all twelve new Batch 389
  ...
  normalized private identifiers and 120 formatted variants across 24,511
  ...
  credential scan found zero nonblank NARA keys and no local secret environment
  file. All 130 public-manifest source/build checks passed for 65 assets
  totaling 82,156,915 bytes. All four distinct Batch 389 citation endpoints
...
- Batch 383 complete browser and accessibility matrix: **1,374 / 1,374
  ...
  deterministic 200-profile audit. Extraction validation passed all 522 pages
  and 23,978 rows. The public-identifier audit examined 12,926 normalized
  private identifiers and 120 formatted variants across 24,505 production
  artifacts, rejected 1,065 substring coincidences, and found zero aggregate,
  manifest-size, or unexpected boundary matches. The direct current-batch
  private-value audit and tracked-source credential scan also found zero
  matches. All 130 public-manifest source/build checks passed for 65 assets
  totaling 81,731,152 bytes. All seven Batch 383 citation endpoints returned
  HTTP 200. The dependency audit found zero vulnerabilities. Consecutive
  exports reproduced the path-and-content public-data digest
...
- Batch 382 complete browser and accessibility matrix: **1,359 / 1,359
  passed** in 10.1 minutes across desktop, phone, and tablet. The clean,
  uninterrupted run includes all 27 axe cases and all fifteen new Batch 382
  cases; the focused regression separately passed **15 / 15**. The database
  passed `quick_check`, had zero foreign-key violations, and passed the
  deterministic 200-profile audit. Extraction validation passed all 522 pages
  and 23,978 rows. The public-identifier audit found zero aggregate or
  unexpected boundary matches across 24,505 production artifacts; two
  manifest-size coincidences were confined to the expected manifest size
  field. The direct seven-identifier evidence-bundle audit and tracked-source
  credential scan also found zero matches. All 130 public-manifest source/build
  checks passed for 65 assets totaling 81,676,925 bytes. All nine Batch 382
  citation endpoints returned HTTP 200. The dependency audit found zero
  vulnerabilities. Consecutive exports reproduced public-data digest
  `d60565bf6e3b358f29a60aa85c1cccf26d95c0813931975f94557d50643fa0e5`;
  consecutive finalized builds reproduced complete static-tree digest
...
- Batch 381 complete browser and accessibility matrix: **1,344 / 1,344
  passed** in 10.0 minutes across desktop, phone, and tablet. The clean,
  uninterrupted run includes all 27 axe cases and all twelve new Batch 381
  cases; the focused regression separately passed **12 / 12**. The database
  passed `quick_check`, had zero foreign-key violations, and passed the
  deterministic 200-profile audit. Extraction validation passed all 522 pages
  and 23,978 rows. The public-identifier audit found zero aggregate, manifest-
  size, or unexpected boundary matches across 24,504 production artifacts.
  The direct seven-identifier evidence-bundle audit and tracked-source
  credential scan also found zero matches. All 130 public-manifest
  source/build checks passed for 65 assets totaling 81,565,585 bytes. Seven of
  ten Batch 381 citation targets returned HTTP 200 to the automated GET check;
  the three HTTP 403 targets were successfully browser-reviewed and their
  access controls were not bypassed. The dependency audit found zero
  vulnerabilities. Consecutive exports reproduced public-data digest
...
- Batch 380 complete browser and accessibility matrix: **1,332 / 1,332
  passed** in 10.0 minutes across desktop, phone, and tablet. The clean,
  uninterrupted run includes all 27 axe cases and all twelve new Batch 380
  cases; the focused regression separately passed **12 / 12** after two
  ...
  production artifacts. The direct six-identifier evidence-bundle audit and
  tracked-source credential scan also found zero matches. All 130 public-
  manifest source/build checks passed for 65 assets totaling 81,441,815 bytes.
  All six Batch 380 citation endpoints returned HTTP 200. The dependency audit
  found zero vulnerabilities. Consecutive exports reproduced public-data
  digest
...
- Batch 373 complete browser and accessibility matrix: **1,236 / 1,236
  ...
  cases; the focused regression separately passed **9 / 9** after a test-only
  hyphenation expectation was aligned with the correctly rendered public
  copy. The database passed `quick_check`, had zero foreign-key violations,
  and passed the deterministic 200-profile audit. The public-identifier audit
  found zero unexpected boundary matches across 24,502 artifacts. All 130
  public-manifest source/build checks passed for 65 assets totaling 80,973,975
...
- Batch 360 complete browser and accessibility matrix: the 11.6-minute run
  ...
  changed.
- Batch 362 complete browser and accessibility matrix: **1,137 / 1,137
  passed** in 10.0 minutes across desktop, phone, and tablet. The clean,
  uninterrupted run includes all 27 axe cases and all nine new Batch 362
  cases.
- Batch 363 complete browser and accessibility matrix: **1,146 / 1,146
...
- Focused Batch 363 browser regression: **9 / 9 passed** across desktop,
  ...
  institutional pathways, keeps two bounded Army-entry occupations separate
  from employers, exposes two identifier conflicts, preserves unresolved
  same-name candidates, and masks every private identifier.
- Focused Batch 362 browser regression: **9 / 9 passed** across desktop,
  phone, and tablet within the complete matrix. It publishes Abel E. Carle's
  H. J. Carle & Sons work and Bogart Carlaw's Lord & Thomas work only as
  documented prewar employers; keeps Albert H. Carl's, Harley J. Carlile's,
  and Warren F. Carlin's Army-entry occupations separate from employers;
  corroborates Spyros J. Carles without interpreting an unassigned occupation
  code; exposes Earle J. Carleton Jr.'s identifier conflict; masks private
  identifiers; and preserves four archival-review paths.
...
- Focused Batch 331 browser regression: **3 / 3 passed** across desktop,
  phone, and tablet. It preserves the roster-supported Lyman D. Burtch Army
  identity, separates Will Burtin's Army, freelance, teaching, and occupation
...
- Focused Batch 352 browser regression: **3 / 3 passed** across desktop,
  phone, and tablet. It publishes two exact-identifier Army-entry occupation
  categories without turning them into employers or immediate OSS
...
- Focused Batch 356 browser regression: **12 / 12 passed** across desktop,
  phone, and tablet. It publishes five exact-identifier Army occupations
  without inventing employers, keeps two student categories separate from
  employment, preserves the Candel/Candela spelling question, keeps George C.
...
- Focused Batch 358 browser regression: **9 / 9 passed** across desktop, phone,
  ...
  as archival-review cases.
- Focused Batch 365 browser regression: **9 / 9 passed** across desktop,
  phone, and tablet. It publishes two exact-identifier Army-entry occupations
  without inventing employers, labels the Ordnance Department only as a
  military assignment, exposes Benjamin T. Carlton's identifier conflict,
...
- Batch 365 local release verification: **passed**. Two consecutive finalized
  ...
  The complete 24,432-page internal-link check passed and inventoried 49,471
  unique external URLs. All 65 public manifest assets cover 80,490,804 bytes;
  manifest SHA-256 is
  ...
  evidence bundle passed repeated-import idempotence, SQLite integrity and
  foreign-key checks; all 75 Python tests and five subtests, the deterministic
  200-profile audit, the complete **1,164 / 1,164** browser and accessibility
  matrix, four direct HTTP 200 source checks, zero-diagnostic Astro validation,
  a zero-vulnerability production dependency audit, and desktop/phone visual
...
- Batch 363 local release verification: **passed**. Two consecutive finalized
  ...
  The complete 24,431-page internal-link check passed and inventoried 49,470
  unique external URLs. All 65 public manifest assets cover 80,397,478 bytes;
  manifest SHA-256 is
  ...
  bundle passed repeated-import idempotence, SQLite integrity and foreign-key
  checks; all 75 Python tests, the deterministic 200-profile audit, the
  complete **1,146 / 1,146** browser and accessibility matrix, seven direct
  HTTP 200/206 source checks, zero-diagnostic Astro validation, and a zero-
  vulnerability production dependency audit.
- Batch 362 local release verification: **passed**. Two consecutive finalized
  production builds reproduced combined public, generated-data, and static-
  site tree digest
  `6723c84e0e7525837369fbbffca0a42d42fb784e71007cff5c06305a1543c698`.
  The complete 24,426-page internal-link check passed and inventoried 49,461
  unique external URLs. All 65 public manifest assets cover 80,287,053 bytes;
  manifest SHA-256 is
  ...
  source-public and 24,496 built artifacts. The strict evidence bundle passed
  repeated-import idempotence, SQLite integrity and foreign-key checks; all 75
  Python tests, the deterministic 200-profile audit, the complete **1,137 /
  1,137** browser and accessibility matrix, seven direct HTTP 200/206 source
  checks, zero-diagnostic Astro validation, and a zero-vulnerability production
  dependency audit.
...
- Batch 356 local release verification: **passed**. The browser-tested public
  ...
  The complete 24,421-page internal-link check passed and inventoried 49,441
  unique external URLs. All 65 public manifest assets matched their recorded
  sizes and SHA-256 values across 79,792,281 bytes; manifest SHA-256 was
  ...
  regression, the clean **1,083 / 1,083** complete matrix, four direct official
  source checks, and a zero-vulnerability production dependency audit passed.
  No credential file or tracked nonblank credential assignment was present.
...
- Batch 352 local release verification: **passed**. Consecutive public trees
  ...
  The complete 24,421-page internal-link check passed and inventoried 49,439
  unique external URLs. All 65 public manifest assets matched their recorded
  sizes and SHA-256 values across 79,573,356 bytes; manifest SHA-256 was
  `6df49d8f8583d1661e49363197c44f1934c259e3d2c05e567d7f5e227b7dbf23`.
  Boundary-aware public-identifier audits found zero unexpected matches in 68
  source-public and 24,491 built artifacts. The strict evidence bundle passed
  three identical imports, SQLite integrity and foreign-key checks; all 74
  Python tests, the deterministic 200-profile audit, the focused **3 / 3**
  regression, the clean **1,047 / 1,047** complete matrix, four direct official
  source checks, and a zero-vulnerability production dependency audit passed.
...
- Batch 338 local release verification: **passed**. Consecutive public-data
  ...
  zero-vulnerability production dependency audit passed.
- Batch 338 production verification: **passed**. Release
  `3bfdda19b48ba6dbb10740df3dfc113f8cad0635` passed pull-request workflow
  ...
  byte-for-byte across 78,921,264 bytes; manifest SHA-256 was
  `152d3ea148287bba22a12a6ddbef76beee7712aa5133bb15bdeda10147727408`.
  Live coverage exactly reproduced 3,166 attempted people, 452 verified
  affiliations, 193 verified employers, 3,118 archival assessments, 2,484
  public claims, and 2,255 public sources.
...
- Batch 331 local release verification: **passed**. Consecutive public-data
  ...
  The complete 24,412-page internal-link check passed and inventoried 49,385
  unique external URLs. All 65 public manifest assets matched in the source
  and built trees across 78,508,930 bytes; manifest SHA-256 was
...
- Focused Batch 316 browser regression: **3 / 3 passed** across desktop, phone,
  and tablet; the expanded **945 / 945** browser and accessibility matrix also
  passed locally.
- Focused Batch 315 browser regression: **3 / 3 passed** across desktop, phone,
  and tablet; the expanded **942 / 942** browser and accessibility matrix also
  passed locally.
- Focused Batch 314 browser regression: **3 / 3 passed** across desktop, phone,
  and tablet; the expanded **939 / 939** browser and accessibility matrix also
  passed locally.
...
- Focused Batch 309 browser regression: **3 / 3 passed** across desktop, phone,
  ...
  passed locally.
- Focused Batch 308 browser regression: **3 / 3 passed**; the expanded
  **921 / 921** browser and accessibility matrix also passed locally.
...
- Focused Batch 298 browser regression: **3 / 3 passed**; a fresh core-route axe
  matrix passed **27 / 27** across desktop, phone, and tablet. The expanded
  **891 / 891** matrix passed in release CI
...
- Focused Batch 294 browser regression: **3 / 3 passed** after one
  ...
  release CI
- Focused Batch 295 browser regression: **3 / 3 passed** after the fixture was
  corrected to use the stable person IDs; the expanded **882 / 882** matrix
  passed in release CI
...
- Focused Batch 291 browser regression: **3 / 3 passed**
- Core-route accessibility scans for Batch 291: **27 / 27 passed** across
  desktop, phone, and tablet, with no serious or critical axe violation; the
  complete **870-case** matrix runs in release CI
...
- Batch 308 local release checks: **passed**; 74 Python tests plus five
  ...
  200-profile audit, database integrity checks, public-identifier audit, and
  complete **921 / 921** browser and accessibility matrix all pass.
  Bronfenbrenner's Army Air Corps pathway and Michigan student status remain
  distinct from civilian employment; Brogger's lawyer occupation names no firm;
  Broh's freelance journalism and Brolliar's Army medical assignments retain
  explicit temporal qualifications; six unsupported or probable profiles remain
  on identity or archival review paths.
...
- Batch 305 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, the complete **912 / 912** browser and
  accessibility matrix, the 24,396-route link check, deterministic rebuild,
  manifest audit, dependency audit, 200-profile audit, database integrity
  checks, and public-identifier audit all pass. Louis R. Alvey Jr., Edwin D.
  Amado, Raymond H. Amador, and Ralph G. Amato have exact-identifier Army
  identities and qualified occupation or student-status findings. Charles D.
  Ambelang's University of Wisconsin student relationship and 1943 officer-
  candidate training remain distinct from civilian employment, and five
  unsupported profiles remain on the archival-review path.
...
- Batch 300 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, **27 / 27** core-route axe scans, the complete
  ...
  audit all pass. Morton L. Bruder and George F. Brule have exact-identifier
  Army identities and qualified occupations; Henry J. Bruman's Pennsylvania
...
- Batch 299 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, **27 / 27** core-route axe scans, the complete
  ...
  audit, 200-profile audit, database integrity checks, and public-identifier
  audit all pass. Six people have exact-identifier Army identities and
  qualified pre-entry occupations or student status. Herbert Brucker's
  immediate 100th Infantry Division-to-OSS pathway is published as a military
  assignment; Evangeline R. Bruciak's 1939 Social Security Board affiliation
  remains visibly probable and medium-confidence; three profiles remain
  archival-review cases. The expanded **894 / 894** browser matrix passed in
  release workflow
  [32207156601](https://github.com/therealjameswilson/before-oss/actions/runs/32207156601).
- Batch 298 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, **27 / 27** core-route axe scans, the complete
  ...
  archival-review cases. The expanded **891 / 891** browser matrix passed in
  release workflow
...
- Batch 297 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, **27 / 27** core-route axe scans, the complete
  24,388-route link check, deterministic rebuild, manifest audit, dependency
  audit, 200-profile audit, database integrity checks, and public-identifier
  audit all pass. John P. Browne and Russell V. Brownell Jr. have exact-
...
- Batch 296 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, the complete 24,388-route link check,
  ...
  database integrity checks, and public-identifier audit all pass. Warren G.
  Brown has an exact-identifier enlisted Army identity without an inferred
  occupation; Willis K. Brown's direct name conflict remains visible; Gordon
  H. Browne receives only a qualified prewar occupation without an invented
  company or immediate chronology; and seven unresolved profiles remain on
  the archival-review path. The expanded **885 / 885** browser matrix passed
  in release workflow
  [32201346414](https://github.com/therealjameswilson/before-oss/actions/runs/32201346414).
- Batch 295 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, the complete 24,388-route link check,
  deterministic rebuild, manifest audit, dependency audit, 200-profile audit,
  database integrity checks, and public-identifier audit all pass. Robert E.
  Brown has an exact-identifier Army identity and qualified residual foreman
  category without an invented employer. Sevellon Brown has an explicit,
  cited Providence Journal-Bulletin-to-OSS pathway; the organization projection
  lists him once while retaining both the immediate-affiliation and last-
  civilian-employer claims. Eight profiles require archival review, and the
  unlinked Dawes Team namesake remains unassigned. The expanded 882-case
...
- Batch 291 local release checks: **passed**; 74 Python tests, the focused
  **3 / 3** browser regression, **27 / 27** core-route axe scans, the complete
  ...
  audit, 200-profile audit, database integrity checks, and public-identifier
  audit all pass. John D. Brown, John L. Brown, Keith J. Brown, and Kenneth M.
  Brown have exact or fixed-width-normalized identifier-backed Army identities;
  three qualified pre-entry occupation categories are published without
  invented employers. Six profiles remain archival-review cases. Undefined
  occupation code 590 is withheld, and the two adjacent John D. Brown rows
  remain separate in a duplicate-review group. The complete 870-case browser
